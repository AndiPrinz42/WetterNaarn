    ...
    .then((data) => {
      for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < FIELDS.length; j++) {
          eval(
            FIELDS[j] + "[" + i + "] = data[" + i + "]['" + FIELDS[j] + "'];"
          );
        }
      }
      date_time = dateTimeCombine(date, time);
      updateTable();
      updateChart();
...
function saveInputValues() {
  dataArea[0] = WEEK.value.slice(WEEK.value.indexOf("-") + 2);
  dataArea[1] = WEEK.value.slice(0, WEEK.value.indexOf("-"));
}
...
const SENSOR_SELECTOR = document.querySelector("#sensors");
...
DONWLOAD_BTN.onclick = function () {
  if (isCurrentChart) {
    downloadChart(
      "WetterNaarnDiagramm_" + "w" + dataArea[0] + "j" + dataArea[1] + ".png"
    );
  } else {
    htmlToCSV(
      "WetterNaarnTabelle_" + "w" + dataArea[0] + "j" + dataArea[1] + ".csv"
    );
  }
};
...
// set week pickers maximum to yesterday
let addZero = "";
let week_number = getWeekNumber(subtractDays(1, new Date()));
if (week_number[0] < 10) {
  addZero = "0";
}
week_number = week_number[1] + "-W" + addZero + week_number[0];
WEEK.setAttribute("max", week_number);

// set week pickers minimum to 28 days before current week
addZero = "";
week_number = getWeekNumber(subtractDays(28, new Date()));
if (week_number[0] < 10) {
  addZero = "0";
}
week_number = week_number[1] + "-W" + addZero + week_number[0];
WEEK.setAttribute("min", week_number);

// set week picker to yesterday
addZero = "";
week_number = getWeekNumber(subtractDays(1, new Date()));
if (week_number[0] < 10) {
  addZero = "0";
}
week_number = week_number[1] + "-W" + addZero + week_number[0];
