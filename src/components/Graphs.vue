  ...
  <!-- <section>
    ...
    <div id="timespanButtons">
      <div id="predefined">
        <div class="custombutton" (click)="timespanSetDay()" [ngClass]="{ selected: timespanButtonsSelected[0] }">
        ...
        <div class="custombutton" (click)="timespanSetWeek()" [ngClass]="{ selected: timespanButtonsSelected[1] }">
        ...
        <div id="month" class="custombutton" (click)="timespanSetMonth()"
          [ngClass]="{ selected: timespanButtonsSelected[2] }">
      ...
      <div id="custom">
        <div id="customselect" class="custombutton" (click)="timespanSetCustom()"
          [ngClass]="{ selected: timespanButtonsSelected[3] }">
        ...
        <mat-form-field id="datepicker" *ngIf="timespanButtonsSelected[3]" color="primary" (click)="picker.open()">
  ...
  <section id="sensorSelection">
    <h2>Sensoren</h2>
    <mat-chip-listbox multiple [(ngModel)]="selectedSensors" (ngModelChange)="this.selectionChange()">
      <div id="sensorGroups">
    ...
    <mat-form-field id="mobileSensorSelector">
      ...
      <mat-select multiple [(ngModel)]="selectedSensors" (ngModelChange)="this.selectionChange()">
  ...
  <section id="graph">
    <div id="chartWrapper">
      <div id="loading-spinner" *ngIf="(data.dateTime.length === 0 || !showChart)">
        <mat-spinner></mat-spinner>
      </div>
      <canvas id="chart" [style.opacity]="(data.dateTime.length !== 0 && showChart) ? 1 : 0" [style.height]="(data.dateTime.length !== 0 && showChart) ? '' : 0"></canvas>
    </div>
  </section> -->
...
import { GraphData } from 'src/assets/graph-data.dto';
...
export class GraphsComponent {
  ...
  timespanButtonsSelected: boolean[] = [false, false, false, false];
  selectedSensors: string[] = ['temperature'];
  mobileSelectorText: string = '';
  ...
  constructor(private metaService: Meta, private apiService: ApiService) { }
  ...
  setMetaTags(): void {
    this.metaService.addTag({ name: 'description', content: 'Analysiere vergangene Jahre mit präzisen Grafiken und erhalte faszinierende Einblicke in Naarns Langzeit-Wettertrends - von Temperaturverläufen bis zu Niederschlagsdaten.' });
    this.metaService.addTag({ name: 'keywords', content: 'Wetter, Naarn im Machlande, Grafiken, Graphen, Verlauf, Langzeitwerte, Wetterhistorie' })
    this.metaService.addTag({ property: 'og:title', content: 'Wetter Naarn | Grafiken' });
    this.metaService.addTag({ property: 'og:description', content: 'Präzise Grafiken über die Daten der letzten Jahre aus Naarn - von Temperaturverläufen bis zu Niederschlagsdaten.' });
    this.metaService.addTag({ property: 'og:image', content: 'https://wetter-naarn.at/assets/icons/logo/og-image.png' });
    this.metaService.addTag({ property: 'og:url', content: 'https://wetter-naarn.at/grafiken' });
  }
  ...
  normalizeMaxMin() {
    const maxminfields: (keyof GraphData)[] = [
      'outTempMax',
      'outTempMin',
      'appTempMax',
      'appTempMin',
    ...
    const datacp: GraphData = JSON.parse(JSON.stringify(this.data));
  ...
  drawChart() {
    ...
    const units = (context: any) => {
      const FIELDS_UNITS: Record<string, string> = {
        Temperatur: '°',
        'Temperatur Min': '°',
        'Temperatur Max': '°',
        'Gefühlte Temperatur': '°',
  ...
  winddirDegreeToString(degree: number): string {
